...
public class AuthService {
	...
	public void sendNewPassword(String usu_email) {
		
		Optional<Usuario> usuario = usuarioRepository.findByEmail(usu_email);
		if (usuario == null) {
			throw new ObjectNotFoundException("Email não encontrado");
		}
		
		String newPass = newPassword();
		usuario.get().setUsu_senha(bcEncode.encode(newPass));		
		usuarioRepository.save(usuario.get());
		emailService.sendNewPasswordEmail(usuario.get(), newPass);
	}
